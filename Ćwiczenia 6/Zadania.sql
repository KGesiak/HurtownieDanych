...
(
SELECT
	OrderDate,sum(SalesAmount) as Sales
...
with cte as
 (
 select OrganizationKey, sum(Amount) as amount_sum
 from FactFinance
 where year(date) = 2012
 group by OrganizationKey
 )

SELECT d.OrganizationKey, amount_sum, OrganizationName, PERCENT_RANK() over(order by amount_sum) as percental
  from cte
...
with cte as
 (
 select OrganizationKey, stdev(Amount) as amount_std
 from FactFinance
 where year(date) = 2012
 group by OrganizationKey
 )

SELECT d.OrganizationKey, amount_std, OrganizationName, PERCENT_RANK() over(order by amount_std) as percental
  from cte
  join DimOrganization d on cte.OrganizationKey = d.OrganizationKey
  order by percental
